...
class NewsChannelArgumentType extends ArgumentType {
	constructor(client) {
	...
	validate(val, msg, arg) {
		...
		return channels.size <= 15 ?
			`${disambiguation(
				channels.map(chan => escapeMarkdown(chan.name)), 'news channels', null
			)}\n` :
...
function channelFilterExact(search) {
	return chan => chan.type === 'news' && chan.name.toLowerCase() === search;
}

function channelFilterInexact(search) {
	return chan => chan.type === 'news' && chan.name.toLowerCase().includes(search);
}

module.exports = NewsChannelArgumentType;