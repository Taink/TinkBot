...
module.exports = function parseLink(link, prefix = 't!') {
	const gameUrl = new URL(link);
	const reSteam = /^\/app\/([0-9]+)\/(\w+)\/$/i;
	const reEpic = /^\/store\/[A-Za-z0-9_-]{2,}\/(?:product|p|bundles)\/([A-Za-z0-9_-]+)(?:\/[A-Za-z0-9_-]+)?$/i;
	const reDiscord = /^\/store\/skus\/([0-9]+)\/([A-Za-z0-9_-]+)$/i;
	const path = gameUrl.pathname;
	let gamename, provider, thumbnail;

	if (gameUrl.href.startsWith('https://www.humblebundle.com/store/')) {
		// Humble Bundle
	...
	} else if (gameUrl.href.startsWith('https://www.gog.com/game/')) {
		// GOG game
	...
	} else if (gameUrl.href.startsWith('https://www.gog.com/movie/')) {
		// GOG movie
		gamename = path.substr(7);
		provider = 'GOG';
		thumbnail = { url: 'https://i.imgur.com/fMC8MRV.png' };
		// TODO: support custom thumbnail for GOG (movies) using HTTPS requests (most probably)
	} else if (gameUrl.href.startsWith('https://store.steampowered.com/app/')) {
		// Steam
	...
	} else if (gameUrl.href.startsWith('https://www.epicgames.com/store/')) {
		// Epic Games Store
		...
		embed: {
			...
			fields: [
				...
				},
				{
					...
					value:
						"Aucun problème, c'est à ça que sert la commande t!invite. Pour voir le code du bot, faites t!github.",
					inline: false,
				},
			],
		},
